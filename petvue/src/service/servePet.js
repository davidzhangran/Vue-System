...
const getPetsByPageSer = async ({ currentPage = 1, eachPage = 3,type,text ,userId} = {}) => {
    if(type!=undefined){
        let data = await fetch(`/serve/getServePetsByPage?currentPage=${currentPage}&eachPage=${eachPage}&type=${type}&text=${text}&userId=${userId}`)
        .then(response => response.json())
        return data
    }else{
        let data = await fetch(`/serve/getServePetsByPage?currentPage=${currentPage}&eachPage=${eachPage}&userId=${userId}`)
        .then(response => response.json())
        return data
    }
}
